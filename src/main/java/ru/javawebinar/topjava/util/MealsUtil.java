...
//                      Collectors.toMap(Meal::getDate, Meal::getCalories, Integer::sum)
    ...
    public static MealWithExceed createWithExceed(Meal meal, boolean exceeded) {
        return new MealWithExceed(id,
                meal.getDateTime(), meal.getDescription(), meal.getCalories(), exceeded);
    }

    public static List<MealWithExceed> getRandomWithExceededByCycle(List<Meal> meals, int caloriesPerDay) {
        return meals.stream().map(meal -> createWithExceed(meal, meal.getCalories() > caloriesPerDay)).collect(Collectors.toList());
    }
}